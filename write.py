...
#
# WRITE - Wigiwys Right IT Editor
# @from Ryan Paul (SegPhault) - 07/12/2009
# @author Arturs Jansons - 

import gtk, webkit, os

class Wygiwys(gtk.Window):
  def __init__(self):
    gtk.Window.__init__(self)
#    self.set_title("Icon")
    self.set_size_request(250, 150)
    self.set_position(gtk.WIN_POS_CENTER)
#    icon_theme = gtk.Icontheme.get.default()
#    pixbuf = gtk_icon_theme_load_icon(icon_theme, "browser", 48, 0, &error)
#    self.set_icon_from_file(pixbuf)
...
#      ("color", gtk.STOCK_SELECT_COLOR, "Select _Color", None, None, self.on_select_color),
      ...
      ("indent", gtk.STOCK_INDENT, "_Indent", None, None, self.on_action),
...
#          "document.execCommand('insertImage', null, '%s');" % fn)
        "document.execCommand('insertImage', null, '%s');" % entry.get_text())
    dialog.destroy()
